...
WITH platform_avg AS (
    SELECT g.*,
        AVG(rating) OVER (PARTITION BY platform) AS avg_platform_rating
    FROM Games g)
SELECT * FROM platform_avg
WHERE rating > avg_platform_rating; 
--------------------------------
--------------------------------
SELECT title, rating
FROM Games
...
SELECT 
    ...
    rating,
    CASE 
        WHEN rating > AVG(rating) OVER (PARTITION BY platform) THEN 'Elite'
        WHEN rating > 3 THEN 'Strong'
        ELSE 'Average'
    END AS Performance
FROM games;
